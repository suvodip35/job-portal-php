...
<h1 class="text-2xl font-bold mb-4 dark:text-white">Edit Job</h1>
...
<form method="post" class="space-y-4" enctype="multipart/form-data">
  ...
  <div>
    <label class="block dark:text-gray-300">Job Title
      <input required name="job_title" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" value="<?= e($job['job_title']) ?>">
    </label>
  </div>
  
  <div class="grid grid-cols-2 gap-4">
    <label class="dark:text-gray-300">Company Name
      <input name="company_name" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" value="<?= e($job['company_name']) ?>">
    </label>
    <label class="dark:text-gray-300">Location
      <input name="location" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" value="<?= e($job['location']) ?>">
    </label>
  </div>
    ...
    <label class="dark:text-gray-300">Category
      <select name="category_slug" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white">
        ...
        <?php foreach ($cats as $c): ?>
          <option value="<?= e($c['category_slug']) ?>" <?= $job['category_slug'] == $c['category_slug'] ? 'selected' : '' ?>><?= e($c['category_name']) ?></option>
        <?php endforeach; ?>
      </select>
    </label>
    <label class="dark:text-gray-300">Minimum Salary ($)
      <input type="number" name="min_salary" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" value="<?= e($job['min_salary']) ?>" min="0">
    </label>
    <label class="dark:text-gray-300">Maximum Salary ($)
      <input type="number" name="max_salary" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" value="<?= e($job['max_salary']) ?>" min="0">
    </label>
  </div>
  
  <div>
    <label class="dark:text-gray-300">Description (Markdown supported)
      <textarea id="markdown-editor" name="description" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" rows="6"><?= e($job['description']) ?></textarea>
    </label>
  </div>
  
  <div>
    <label class="dark:text-gray-300">Requirements
      <textarea name="requirements" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" rows="4"><?= e($job['requirements']) ?></textarea>
    </label>
  </div>
  
  <div class="grid grid-cols-3 gap-4">
    <label class="dark:text-gray-300">Apply URL
      <input name="apply_url" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" value="<?= e($job['apply_url']) ?>">
    </label>
    <label class="dark:text-gray-300">Document Link (PDF, DOC, etc.)
      <input name="document_link" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" value="<?= e($job['document_link']) ?>">
    </label>
    <label class="dark:text-gray-300">Last Date
      <input type="date" name="last_date" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" value="<?= e($job['last_date']) ?>">
    </label>
  </div>
  ...
  <div class="grid grid-cols-3 gap-4">
    ...
    <label class="dark:text-gray-300">Meta Title
      <input name="meta_title" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" value="<?= e($job['meta_title']) ?>">
    </label>
  </div>
  
  <div>
    <label class="dark:text-gray-300">Meta Description
      <textarea name="meta_description" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-700 dark:text-white" rows="2"><?= e($job['meta_description']) ?></textarea>
    </label>
  </div>
  ...
  <div class="space-y-2">
    ...
    <p class="text-xs text-gray-500 dark:text-gray-400">Max 2MB (JPG, PNG, WEBP). Leave empty to keep current image.</p>
  </div>
  
  <div class="flex justify-end">
    <button class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-800">Update Job</button>
  </div>
</form>
