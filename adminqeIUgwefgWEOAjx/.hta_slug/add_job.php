...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            // If error is missing column (e.g. suggested_books), attempt ALTER TABLE or retry without suggested_books
            if (strpos($e->getMessage(), 'suggested_books') !== false || $e->getCode() == '42S22') {
                try {
                    ...
                    try {
                        ...
                        $err = "Database Insert Error: " . $ex2->getMessage();
                    }
                }
            } else {
        ...
        if ($inserted) {
            ...
            if ($status === 'published') {
                try {
                    ...
                    $success .= ' (Push notification: ' . $e->getMessage() . ')';
                }
            }
        }
    } else {
