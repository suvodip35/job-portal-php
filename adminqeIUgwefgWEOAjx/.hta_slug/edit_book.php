...
$book_id = $_GET['id'] ?? '';
if (!$book_id) {
    echo "<script>window.location.href='/adminqeIUgwefgWEOAjx/books'</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($title !== $book['title']) {
            $base_slug = slugify($title);
            $new_slug = unique_slug($pdo, 'books', 'slug', $base_slug, $book['id']);
        }
        
        $stmt = $pdo->prepare("UPDATE books SET title = ?, slug = ?, book_type = ?, description = ?, author = ?, publisher = ?, publication_year = ?, isbn = ?, amazon_link = ?, flipkart_link = ?, book_image = ?, meta_title = ?, meta_description = ?, updated_at = NOW() WHERE id = ?");
        $stmt->execute([$title, $new_slug, $book_type, $description, $author, $publisher, $publication_year, $isbn, $amazon_link, $flipkart_link, $book_image, $meta_title, $meta_description, $book_id]);
...
<h1 class="container mx-auto text-2xl font-bold mb-4 dark:text-white">Edit Book</h1>
...
<form method="post" class="space-y-6 container mx-auto" accept-charset="UTF-8" enctype="multipart/form-data">
  ...
  <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
    ...
    <!-- Current Book Image Preview -->
    <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
        <label class="block text-sm font-medium mb-2 dark:text-gray-300">Current Book Cover</label>
        <div class="flex items-center gap-4">
            <img src="<?= getBookImage($book['book_image'], $book['title']) ?>" 
                 alt="<?= e($book['title']) ?>" 
                 class="w-24 h-32 object-cover rounded border dark:border-gray-600">
            <div class="text-sm text-gray-600 dark:text-gray-400">
                <p class="font-medium"><?= e($book['title']) ?></p>
                <p class="mt-1">Upload a new image to replace the current cover.</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <input type="url" name="amazon_link" id="amazon_link" 
                   ...
                   value="<?= e($_POST['amazon_link'] ?? $book['amazon_link']) ?>" 
        ...
        <div>
            ...
            <input type="url" name="flipkart_link" id="flipkart_link" 
                   ...
                   value="<?= e($_POST['flipkart_link'] ?? $book['flipkart_link']) ?>" 
    ...
    <div class="mb-4">
        ...
        <textarea id="markdown-editor" name="description" 
                  ...
                  rows="6" placeholder="Provide detailed description about the book, key features, topics covered, etc..."><?= e($_POST['description'] ?? $book['description']) ?></textarea>
  ...
  <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
    ...
    <div class="mb-4">
        ...
        <input type="text" name="meta_title" id="meta_title" 
              class="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white"
              value="<?= e($_POST['meta_title'] ?? $book['meta_title']) ?>" 
    ...
    <div class="mb-4">
        ...
        <textarea name="meta_description" id="meta_description" 
                  class="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white" 
                  rows="4" placeholder="Complete guide for competitive exam preparation. Recommended by experts and toppers..."><?= e($_POST['meta_description'] ?? $book['meta_description']) ?></textarea>
    </div>
  </div>

  <!-- Book Metadata -->
  <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
    <h2 class="text-lg font-semibold mb-4 text-gray-800 dark:text-white border-b pb-2">Book Metadata</h2>
    
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        <div>
            <p class="text-gray-600 dark:text-gray-400"><strong>Slug:</strong> <?= e($book['slug']) ?></p>
            <p class="text-gray-600 dark:text-gray-400"><strong>Created:</strong> <?= date('M d, Y H:i', strtotime($book['created_at'])) ?></p>
        </div>
        <div>
            <p class="text-gray-600 dark:text-gray-400"><strong>Last Updated:</strong> <?= date('M d, Y H:i', strtotime($book['updated_at'] ?? $book['created_at'])) ?></p>
            <p class="text-gray-600 dark:text-gray-400"><strong>Status:</strong> 
                <span class="px-2 py-1 text-xs rounded <?= $book['status'] === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' ?>">
  ...
  <div class="flex justify-between gap-3">
    <div class="flex gap-3">
        <a href="/adminqeIUgwefgWEOAjx/books" class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600 flex items-center">
