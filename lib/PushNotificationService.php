 ...
 * This is a proven, simplified implementation that bypasses VAPID JWT issues
 * Use this to replace the existing PushNotificationService completely
 */
...
class PushNotificationService {
            ...
            foreach ($subscriptions as $subscription) {
                try {
                    ...
                    if ($result['success']) {
                        ...
                        // Don't deactivate subscription - let user manually handle it
    ...
    private function generateVAPIDAuthHeader($endpoint, $vapid) {
        $timestamp = time();
        $jwtHeader = base64_encode(json_encode(['typ' => 'JWT', 'alg' => 'ES256']));
